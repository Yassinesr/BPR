"""BPR refinement with 8-view TTA (full D4 symmetry group).

Identical to inference_img.py except for the _inference_one function inside
inference(), which runs the model on 8 augmented views of each patch:
  - identity
  - H-flip
  - V-flip
...
Predictions are un-augmented and averaged. Use this script in place of
inference_img_tta.py when you want the full D4 group instead of just flips.

Note: rotations are only safe when patches are square. BPR uses 256x256
patches so this is fine.
"""
import os
...
def collect_test_paths(root, pred_root):
    """Return image and coarse-mask paths for all test datasets."""
    img_paths, dt_paths = [], []
...
# ======================================================
def split(img, maskdts, boundary_width=3, iou_thresh=0.55,
          patch_size=64, out_size=256):
...
def inference(cfg, ckpt, img_paths, dt_paths, out_dir, max_ins=32, device='cpu'):
    ...
    def _inference_one(img, sub_maskdts, sub_dt_paths):
        ...
        # ===== 8-view D4-group TTA =====
        # patches: (N, C, H, W). Spatial dims for INPUT are [2, 3].
        # _run_chunked output: (N, H, W). Spatial dims for OUTPUT are [1, 2].
        ...
        # 5. 90° CCW rotation. To undo, rotate the output 90° CW (k=-1).
        out = _run_chunked(torch.rot90(patches, k=1, dims=[2, 3]))
        acc = acc + torch.rot90(out, k=-1, dims=[1, 2])
        del out

        # 6. 90° CCW + H-flip
        rotated = torch.rot90(patches, k=1, dims=[2, 3])
        out = _run_chunked(torch.flip(rotated, dims=[3]))
        # Undo: first H-flip (dims=[2] for output), then rotate 90° CW
        acc = acc + torch.rot90(torch.flip(out, dims=[2]), k=-1, dims=[1, 2])
        del out, rotated

        # 7. 270° CCW rotation (= 90° CW). Undo with 90° CCW (k=1).
        out = _run_chunked(torch.rot90(patches, k=-1, dims=[2, 3]))
        acc = acc + torch.rot90(out, k=1, dims=[1, 2])
        del out
        ...
        refinemasks_final = acc / 8.0   # (N, H, W)

        # ===== Merge refined patches back into full masks =====
...
if __name__ == '__main__':
    ...
    # Choose ONE checkpoint by uncommenting it:
    # ckpt = "../work_dirs/hrnet48_256/latest.pth"          # vanilla BPR
    # ckpt = "../work_dirs/hrnet48_256_fct/latest.pth"      # FCT(0.3, vflip) checkpoint
    ckpt = "../work_dirs/hrnet48_256_fct_noflip/latest.pth"  # Exp 4 (no flip aug)

    # Output folder name should describe the run for easy bookkeeping
    out_tag = "FCTnoflip"   # change to describe this run

    model = _build_model(cfg, ckpt)
    for test in ["PraNet"]:
        test_root = "../dataset/TestDataset"
        pred_root = "../dataset/PraNet_preds"
        out_dir = "./results/poly_BPR_" + out_tag
