...
# CONFIGURATION
# ========================
root_dir = "./ValidationDataset"       
pred_dir = "./ValidationDataset"           
...
pred_path = os.path.join(root_dir, "predval")

file_list = [f for f in os.listdir(gt_path) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]

# === TEMPORARY: sample a small subset for quick testing ===
# This block selects 10 random files from the dataset so you can run the
# script quickly. It is intended to be removed after testing — look for
# the marker above to delete it.
# import random
# _RANDOM_TEST_SAMPLE_SIZE = 10
# random.seed(52)  # reproducible
# if len(file_list) > _RANDOM_TEST_SAMPLE_SIZE:
#     file_list = random.sample(file_list, _RANDOM_TEST_SAMPLE_SIZE)
# === END TEMPORARY BLOCK ===

# Output paths
out_img_dir = os.path.join(output_dir, "img_dir", "val")
out_gt_dir = os.path.join(output_dir, "ann_dir", "val")
out_pred_dir = os.path.join(output_dir, "mask_dir", "val")
...
for filename in file_list:
    ...
    # === TEMPORARY: save the patches for inspection ===
    # Set this to False to skip saving; remove the whole block after testing.
